...
from yt_sabr2.core.UMP import UMP
from yt_sabr2._shim_v2 import Monostate
from yt_sabr2._shim_v2 import SABRError
from yt_sabr2._shim_v2 import open_request
from yt_sabr2.video_streaming.sabr_error import SabrError
from yt_sabr2.video_streaming.media_header import MediaHeader
from yt_sabr2.core.chunked_data_buffer import ChunkedDataBuffer
from yt_sabr2.video_streaming.sabr_redirect import SabrRedirect
from yt_sabr2.video_streaming.playback_cookie import PlaybackCookie
from yt_sabr2.video_streaming.next_request_policy import NextRequestPolicy
from yt_sabr2.video_streaming.streamer_context import StreamerContextUpdate
from yt_sabr2.video_streaming.stream_protection_status import StreamProtectionStatus
from yt_sabr2.video_streaming.video_playback_abr_request import VideoPlaybackAbrRequest
from yt_sabr2.video_streaming.format_initialization_metadata import FormatInitializationMetadata
...
class ServerAbrStream:
    ...
    def start(self):
        ...
        max_duration_ms = getattr(self.stream, 'max_duration_ms', 0)
        while client_abr_state['playerTimeMs'] < self.totalDurationMs:
            if max_duration_ms and client_abr_state['playerTimeMs'] >= max_duration_ms:
                # 预览场景：到达时长上限，在完整片段边界处停止（文件包含完整 init + 媒体分片）
                break
            try:
                data = self.fetch_media(client_abr_state, audio_format, video_format)
            except Exception:
                # 单个 POST 失败（超时/连接中断/证书抖动）：reload 续传，不中断整体下载
                self.reload()
    ...
    def fetch_media(self, client_abr_state, audio_format, video_format):
        body = VideoPlaybackAbrRequest.encode({
            ...
            'streamerContext': {
                ...
                'clientInfo': getattr(self.stream, '_abr_client_info', None) or {
                    'clientName': 1,
        ...
        return self.parse_ump_response(bytes(open_request(request, timeout=5).read()))
    ...
    def reload(self):
        ...
        import sys as _sys
        print(f"SABR_RELOAD attempts_left={self.maximum_reload_attempt} status={self.stream_protection_status}", file=_sys.stderr, flush=True)
