...
- **YouTube Cookie 与格式不一致**：公开视频固定匿名探测和匿名下载；仅当匿名探测明确提示登录/访问受限、且 Cookie 复探成功时，才在下载中继承该已授权会话，避免 Android VR client 携带 Cookie 时出现「Requested format is not available」。
- **剪贴板自动下载未执行**：单链接分析完成后会按设置的统一格式策略解析格式并调用现有下载入口；无可用格式时记录跳过原因。
- **TLS 证书校验失败**：下载阶段遇到 `CERTIFICATE_VERIFY_FAILED` 时，不再弹确认框，自动仅对当前任务以兼容 TLS 模式重试一次。
- **B 站 DASH 音频 CDN 断连**：视频流已完成而音频出现 `Remote end closed connection`、连接重置或读取中断时，保留已下载视频并在短暂退避后仅重试音频一次；失败提示改为可读的 CDN 连接中断说明。
- **默认保存方式显示错位**：保存方式菜单使用同一固定顺序渲染和回写，修复选择「相册」却被保存为「文件」的问题。

### 验证

- TypeScript 诊断 0 错误；`scripting-ts project "Yoinks"` 启动回归通过。
- 真机验证通过：YouTube 授权状态继承、剪贴板自动下载、TLS 自动兼容重试，以及 B 站音频 CDN 断连后重试并合并完成。

## 1.5.0 - 2026-07-29

### 新增

- **在线播放器控制**：HLS 与 DASH 预览支持 `0.5x`–`2.0x` 倍速；视频、音频分离的双流预览会同步调整两条轨道的播放速度。
- **HLS 画质选择**：当媒体提供多个清晰度时，可在播放器中选择「自动」或指定清晰度；单档和直链播放不会显示无效入口。
- **YouTube 按需登录**：公开视频保持匿名探测；遇到登录或会员专享等访问限制时，可登录 YouTube 后重试。设置页支持导入 Netscape 格式的 `cookies.txt`。
- **发现页**：支持展开播放列表、合集、频道和作者主页，并将选中内容批量加入下载队列；关键词搜索与相关推荐可在设置中作为实验功能启用。

### 兼容性与修复

- YouTube 探测和下载优先使用 `android_vr` player client，改善缺少 JavaScript runtime 时的格式解析。
- X（Twitter）多视频帖自动定位到 `/video/1`，修复裸状态链接无法列出格式或下载格式不匹配的问题。
- X 在线预览优先选择带音频的 progressive MP4，改善 HLS video-only 流无声的问题。

### 验证

- TypeScript 诊断 0 错误；`verify_player_controls.ts` 13/13；`verify_x_multivideo_probe.ts` 7/7；`verify_x_preview_audio.ts`、发现页相关验证和 Python 脚本编译均通过。
- 仍建议在真机验证：多档 HLS 画质切换、双流倍速同步，以及 YouTube 会员内容登录重试闭环。
