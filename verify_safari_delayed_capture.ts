...
const checks: Array<[string, boolean]> = [
  ...
  ["top-level capture waits before scanning candidates", /await wait\(CAPTURE_DELAY_MS\)\s*\n\s*const topLevel = await collectCandidates\(\)/.test(source)],
  ["frames wait before their local scan", /async function reportFrame[\s\S]*await wait\(CAPTURE_DELAY_MS\)[\s\S]*candidates: await collectCandidates\(\)/.test(source)],
  ...
  ["version bumped to 1.2.3", /\/\/ @version 1\.2\.3/.test(source)],
]
