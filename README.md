...
- **Safari 候选采集**：Safari 用户脚本 **1.1.6** 仅在用户主动操作时从当前页采集公开媒体候选；浮动入口可**按住拖动放置**到屏幕任意边缘（松手自动吸附左/右边缘并记忆位置，不再固定右下角遮挡网页内容），短按入口或菜单会等待约 1.5 秒，减少播放器异步初始化的遗漏；长按入口可移除（关闭“始终显示浮动入口”时）；单候选直接进入分析，多候选点选后直接分析；采集直链优先保留，页面探测失败时可携带 Referer 回退；支持无扩展名 HLS 端点、跨域 iframe 正片与“点击播放后才请求”的站点；采集结果给出三种反馈（已捕获 / 需要解析 / 未捕获），顶层无候选时保存公开播放器 iframe 线索。
